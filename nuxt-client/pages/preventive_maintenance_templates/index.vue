<template>
  <div>
    <div class="mb-5">
      <v-row>
        <v-col>
          <h2>Preventive Maintenance Templates</h2>
        </v-col>
        <v-col>
          <v-btn small color="info" style="float: right" @click="addPreventiveMaintenanceTemplate">Add Preventive Maintenance Template</v-btn>
        </v-col>
      </v-row>
    </div>
    ...
    <v-card>
      ...
      >
        ...
        <template v-slot:item.actions="{ item }">
          <v-tooltip top>
            ...
            <span>Manage Preventive Maintenance Template</span>
          </v-tooltip>
        </template>
      ...
      <ManagePreventiveMaintenanceTemplateComponent
        v-if="addingPreventiveMaintenanceTemplate"
        v-model="addingPreventiveMaintenanceTemplate"
...
export default {
  ...
  data: () => ({
    loading: true,
    addingPreventiveMaintenanceTemplate: false,
    filterByName: null,
    headers: [
      // { text: 'Preventive Maintenance Template ID', align: 'left', value: 'id' },
      { text: 'Name', align: 'left', value: 'name' },
      { text: 'Recurring', align: 'left', value: 'recurring' },
      { text: 'PM Due Type', align: 'left', value: 'system_p_m_due_type.name' },
  ...
  methods: {
    async getDataFromApi(options) {
      ...
      const response = await this.$axios.$get('preventive_maintenance_templates', {
        params: {
    ...
    async openData (dataItem) {
      this.$router.push(`/preventive_maintenance_templates/${dataItem.id}`)
    },
    clearFilters () {
      this.filterByName = null
      this.getDataFromApi()
    },
    addPreventiveMaintenanceTemplate () {
      this.addingPreventiveMaintenanceTemplate = true
    }
  }
}
</script>
